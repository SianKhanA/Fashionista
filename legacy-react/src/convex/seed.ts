...
export const seedCoupons = internalMutation({
  args: {},
  handler: async (ctx) => {
    const coupons = [
      { code: "WELCOME10", type: "percentage" as const, value: 10, minOrder: 5000, maxUses: 500 },
      { code: "FREESHIP", type: "free_shipping" as const, value: 0 },
    ];
    for (const coupon of coupons) {
      const existing = await ctx.db.query("coupons").withIndex("by_code", (q) => q.eq("code", coupon.code)).unique();
      if (!existing) await ctx.db.insert("coupons", { ...coupon, usedCount: 0, active: true });
    }
    return "Coupons ready";
  },
});
