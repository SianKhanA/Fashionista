...
export const list = query({
  ...
  handler: async (ctx, args) => {
    let products = ctx.db.query("products").withIndex("by_active", (q) => q.eq("active", true));
    ...
    if (args.featured) {
      products = ctx.db.query("products").withIndex("by_featured", (q) =>
        q.eq("featured", true)
      );
    }

    const allProducts = await products.collect();
    ...
    return { products: paginated, total, hasMore: offset + limit < total };
  },
});
